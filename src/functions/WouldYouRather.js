...
const res = await (await (fetch(`https://api.tovade.xyz/v1/fun/wyr`))).json();
...
await message.channel.send('**Would you rather**',{ components: [{type:1, components:[btn, btn2]}]})
.then( async(m) => {
bruh.delete()
...
.setDisabled()
gameCollector.stop()
m.edit('**Would you rather**',{ components: [{type:1, components:[btn, btn2]}]})
}else if(ae.id === id2){
btn = new disbut.MessageButton()
.setStyle('gray')
.setLabel(`${res.questions[0]}` + ` (${res.percentage['1']}%)`)
