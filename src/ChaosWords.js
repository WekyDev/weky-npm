...
module.exports = async (options) => {
	...
	try {
		...
		let btn1 = new MessageButton()
			...
			.setCustomId(id);
		const mes = await options.message.channel.send({
			embeds: [embed]
		});
		await mes.edit({
			embed,
		...
		game.on('collect', async (msg) => {
			const condition = options.words.includes(msg.content.toLowerCase()) && !guessed.includes(msg.content.toLowerCase());
			if (condition) {
				remaining++;
		...
		const filter2 = interaction => interaction.customId == id && interaction.user.id == options.message.member.id;
		const gameCollector = mes.channel.createMessageComponentCollector({
			filter2
		});
